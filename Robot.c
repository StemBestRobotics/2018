...
#pragma config(Motor,  port6,           clawAngle1Servo, tmotorServoStandard, openLoop)
#pragma config(Motor,  port7,           clawAngle2Servo, tmotorServoStandard, openLoop)
#pragma config(Motor,  port8,           doorServo,     tmotorServoStandard, openLoop)
#pragma config(Motor,  port9,           clawGrabServo, tmotorServoStandard, openLoop)
...
Servos: 2
- buttons need to control the angle of the servos controlling the scoop
...
void clawServos(){											 								//CLAW: Allows Controller to control movement of claw
	while (vexRT[Btn6U] && claw < 127) {
		motor[clawAngle1Servo] = claw;
		motor[clawAngle2Servo]= -claw; 
		claw += 10;	
		wait1Msec(10);                	
	}
	while (vexRT[Btn6D] && claw > -127) {
		motor[clawAngle1Servo] = claw;
		motor[clawAngle2Servo]= -claw; 
		claw -= 10;	
		wait1Msec(10);  
	...
	if(vexRT[Btn7D] && clawGrabCheck){										//BUTTON true: Checks if button 7D is being pressed, and if clawGrabCheck is true, if they are both true:
		...
		if(clawGrabToggle){																		//TOGGLE true: Checks if clawGrabToggle is true, if it is:
 			...
 			wait1Msec(100);//Toggles the clawGrabToggle boolean to false, so it can toggle from false to true next time
			...
			clawGrabToggle = true;
			wait1Msec(100);//Toggles the clawGrabToggle boolean to true, so it can toggle from true to false next time
		}
	}
	else if(!vexRT[Btn7D]){
		clawGrabCheck = true;
...
task main(){																						//main start task
	while(true) {																						//gurantees that the cortex will run the following methods indefinintly until it is turned off.
