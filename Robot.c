...
void clawServos(){											 								//CLAW: Allows Controller to control movement of claw
 	if(vexRT[Btn6U]){																				//ANGLE CONTROL checks if button 6U is being pressed, if it is:
		motor[clawAngle1Servo] = 90;
		motor[clawAngle2Servo] = -90;
	}
	if(vexRT[Btn6D]){																			//ANGLE CONTROL checks if button 6D is being pressed, if it is:
		motor[clawAngle1Servo] = -90;
		motor[clawAngle2Servo] = 90;
 	}
